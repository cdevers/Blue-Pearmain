...
- A failed mid-run `index-legacy` leaves the existing `legacy_assets` intact or partially-refreshed-forward (the authoritative reconciliation/deletion pass in `index_library` only executes after a successful full iteration — see `legacy_indexer.py:205–206`).

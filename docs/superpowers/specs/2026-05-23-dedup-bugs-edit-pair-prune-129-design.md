...
> **Heuristic note:** The `DSC_` prefix is a Nikon convention that reliably identifies Snapbridge-eligible files in this collection, but it is a pragmatic heuristic rather than a guarantee. Future camera models, firmware changes, or import paths from different sources could produce `DSC_`-prefixed files that are not Snapbridge pairs, or Snapbridge-like pairs under a different prefix. If edge cases emerge, the filename guard should be revisited — it is the right call for now, not a permanent semantic contract.
...
`--prune` defaults to **dry-run** (reports what would change). Requires `--apply` to execute. Reports three specific counters: **groups deleted** (Class A), **photo links cleared** (dangling `duplicate_group_id` from Class A deletions), and **counts repaired** (Class B photo_count updates). Does **not** re-classify groups or touch `resolved=1` groups.
...
- `test_deduplicator.py`:
  ...
  - `test_prune_invariant_photo_count_matches_linked` — after `--prune --apply`, every unresolved group has `photo_count == COUNT(photos linked)`
  - `test_prune_invariant_no_zombie_groups` — after `--prune --apply`, no unresolved group has fewer than 2 linked photos
  - `test_prune_invariant_no_dangling_group_id` — after `--prune --apply`, no photo has a `duplicate_group_id` pointing to a non-existent group

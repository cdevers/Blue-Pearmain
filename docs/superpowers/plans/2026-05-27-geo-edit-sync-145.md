...
- [ ] **Step 4: Scope `run_sync_engine` broad supersede to text fields only**
...
class TestLibraryGeoUI:
    ...
    def test_no_loc_pill_absent_for_geotagged(self, client_lib):
        # Geotagged photo should not have the pill
        html = client_lib.get("/library").data.decode()
        # Verify pill is not shown for glui-u1 (geotagged)
        # (exact check: count occurrences of no-loc-pill vs geotagged photos)
        assert html.count("no-loc-pill") == 2  # only the 2 unconfirmed untagged photos

    def test_no_loc_pill_absent_for_confirmed_none(self, client_lib):
        # geo_confirmed_none=1 photos should not show pill
        assert client_lib.get("/library").data.decode().count("no-loc-pill") == 2

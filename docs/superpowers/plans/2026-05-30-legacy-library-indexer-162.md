...
1. **Snapshot the pre-interruption UUID set** for the library under test, to a UUID-named file (so snapshots from different libraries/runs never clobber each other):
   Run: `python -c "import sys; sys.path.insert(0,'.'); import yaml; from db.db import Database; c=yaml.safe_load(open('config/config.yml')); d=Database(c['database']['path']); lib=d.conn.execute('SELECT library_uuid FROM legacy_libraries ORDER BY indexed_at DESC LIMIT 1').fetchone()[0]; uu=sorted(r[0] for r in d.conn.execute('SELECT asset_uuid FROM legacy_assets WHERE library_uuid=?', (lib,))); path=f'/tmp/legacy-uuids-before-{lib}.txt'; open(path,'w').write('\n'.join(uu)); print('library:', lib); print('before count:', len(uu)); print('snapshot:', path)"`
   Note the printed `library:` UUID and `snapshot:` path — both are reused in step 3.
...
3. **Snapshot the post-interruption UUID set and assert the before-set is fully preserved** (explicit invariant, not just a count). Substitute `<library_uuid>` with the `library:` value printed in step 1:
   Run: `python -c "import sys; sys.path.insert(0,'.'); import yaml; from db.db import Database; lib='<library_uuid>'; c=yaml.safe_load(open('config/config.yml')); d=Database(c['database']['path']); after=set(r[0] for r in d.conn.execute('SELECT asset_uuid FROM legacy_assets WHERE library_uuid=?', (lib,))); before=set(open(f'/tmp/legacy-uuids-before-{lib}.txt').read().split('\n')) - {''}; missing=before - after; print('before:', len(before), 'after:', len(after), 'missing:', len(missing)); assert not missing, f'INVARIANT VIOLATED: {len(missing)} pre-existing UUIDs were deleted: {sorted(missing)[:10]}'; print('OK: no pre-existing asset_uuid disappeared')"`

...
**Architecture:** A new `poller/exporter.py` module contains pure functions for serialising DB rows and writing output files. `bp` gets a `cmd_export` handler and an `export` subparser. No new DB tables or migrations.

**Output format — NDJSON by default:** Photos are written as NDJSON (`photos.ndjson` — one JSON object per line). NDJSON scales to large archives without loading everything into memory, is grep-able, and is streamable. Zones are a small count so they remain a plain JSON array (`zones.json`). The manifest is always `manifest.json`. YAML is explicitly excluded from this version — it adds long-term format maintenance burden without meaningful benefit over JSON.

**Manifest fields:** `exported_at` (ISO-8601), `photo_count`, `zone_count`, `bp_version` (from `bp.__version__` or `"unknown"`), `export_format_version` (currently `"1"`).

**Tech Stack:** Python stdlib (`json`, `pathlib`), existing `db.db.Database` (raw SQL queries on `photos`, `geofence_zones`, `photo_albums`, `albums`).
...
def collect_export_data(db: "Database") -> dict:
    ...
    # Read BP version from the bp script if available
    try:
        import importlib.util
        spec = importlib.util.spec_from_file_location("bp_version", Path(__file__).parent.parent / "bp")
        # Version string from bp file header comment or __version__ attribute
...
class TestWriteExport(unittest.TestCase):
    ...
    def test_photos_ndjson_has_one_object_per_line(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(len(lines), 2)  # one line per photo
            for line in lines:
...
```python
def cmd_export(args):
    """Export BP database state to portable NDJSON files."""
    import yaml
    ...
    print(f"  photos.ndjson  (NDJSON — one object per line)")
    print(f"  zones.json     (JSON array)")
...
```python
    ...
    p_export = sub.add_parser(
        "export",
        help="Export DB state to portable NDJSON (photos.ndjson, zones.json, manifest.json)",
    )

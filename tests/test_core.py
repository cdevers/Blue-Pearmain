...
class TestMigrate015FriendsFamily(unittest.TestCase):
    ...
    def test_migration_allows_approved_friends_after_run(self):
        ...
        with self.assertRaises(Exception):
            conn.execute("INSERT INTO photos (uuid, privacy_state) VALUES ('x', 'approved_friends')")
        conn.close()
        run(self.db_path)
...
class TestScannerFriendsStates(unittest.TestCase):
    ...
    def _existing(self, state: str) -> dict:
        return {
            "id": 1, "flickr_id": "flickr-scan-001", "uuid": None,
            "privacy_state": state,
    ...
    def test_approved_friends_is_protected_from_overwrite(self):
        from poller.scanner import build_enriched_row
        enriched = build_enriched_row(self._PHOTO_ROW, self._existing("approved_friends"), [], "Alice")
        self.assertEqual(enriched["privacy_state"], "approved_friends",
                         "approved_friends must not be overwritten by scanner")

    def test_approved_family_is_protected_from_overwrite(self):
        from poller.scanner import build_enriched_row
        enriched = build_enriched_row(self._PHOTO_ROW, self._existing("approved_family"), [], "Alice")
        self.assertEqual(enriched["privacy_state"], "approved_family",
                         "approved_family must not be overwritten by scanner")

    def test_approved_friends_family_is_protected_from_overwrite(self):
        from poller.scanner import build_enriched_row
        enriched = build_enriched_row(self._PHOTO_ROW, self._existing("approved_friends_family"), [], "Alice")
        self.assertEqual(enriched["privacy_state"], "approved_friends_family",

...
class TestMapPhotoIdParam:
    ...
    def test_highlight_id_is_null_when_no_photo_id(self, client_geo):
        c, *_ = client_geo
        html = c.get("/map").data.decode()
        assert "highlightId = null" in html
    ...
    def test_falls_back_to_average_when_photo_has_no_coords(self, client_geo):
        ...
        assert "highlightId = null" in html  # highlight_id falls back to None → null
...
class TestMapHighlightJS:
    ...
    def test_marker_by_id_cleared_on_reload(self, client_geo):
        c, *_ = client_geo
        html = c.get("/map").data.decode()
        assert "Object.keys(_markerById).forEach(k => delete _markerById[k])" in html
    ...
    def test_highlight_done_guard_present(self, client_geo):
        c, *_ = client_geo
        html = c.get("/map").data.decode()
        assert "_highlightDone" in html
